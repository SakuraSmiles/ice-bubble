      ...
      <div class="header-top">
        <div class="header-left">
          ...
          <div class="title-area">
            <h1 class="page-title">{{ label || '会话详情' }}</h1>
            <span class="page-subtitle">{{ agentName }}</span>
          </div>
        </div>
        <div class="header-actions">
          <el-tag v-if="status" :type="statusType(status)" size="small" effect="plain">
            {{ statusLabel(status) }}
          </el-tag>
          <span v-if="created" class="meta-time">{{ new Date(created).toLocaleString('zh-CN') }}</span>
        </div>
      </div>
      <ConnectionAlert />
    </header>
